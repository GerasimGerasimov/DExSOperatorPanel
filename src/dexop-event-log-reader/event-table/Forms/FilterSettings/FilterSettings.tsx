...
import { IFilterSettingsProps } from "../../../../interfaces/IFilterSettingsProps";
import { IFilterSettingsState } from "../../../../interfaces/IFilterSettingsState";
...
export default class FilterSettings extends Component<IFilterSettingsProps, IFilterSettingsState> {
  ...
  componentDidMount () {
    ...
    const value: IEventSortMode = this.props.Range.event || IEventSortMode.All;
    const key = getKeyOfEnumByValue(IEventSortMode, value, 'All');
  ...
  private getQuery (): ISearchRangeQuery | undefined {
    ...
    if (this.setFocusOfNotValidElement(useDate, dateFrom, this.refFromDate)) {
      return undefined;
    };
    if (this.setFocusOfNotValidElement(useDate, dateTo, this.refToDate)) {
      return undefined;
    };
    if (this.setFocusOfNotValidElement(useEvent, event, this.refEvent)) {
      return undefined;
    };
  ...
  render () {
    return (
      <div className='search block grid-container'>
        ...
        <button
          className={
            'btn btn-primary btn-xs search Search' +
            this.showIfUsed(this.state.useTimeRangeInSearch || this.state.useEventTypeInSearch)
          }
