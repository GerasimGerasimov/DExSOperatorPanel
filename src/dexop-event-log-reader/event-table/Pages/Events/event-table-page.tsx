...
import { IEventSortMode } from "../../../event-models/events/sort-modes";
...
import { ISearchRangeQuery } from "../../../../interfaces/ISearchRangeQuery";
...
export default class EventTablePage extends Component <RouteComponentProps<IEventsProps>, IEventsState> {
  ...
  private eventsDate: string = '';
  private callback: IOnChangeCallback | undefined;

  constructor (props : RouteComponentProps<IEventsProps>) {
    super(props);
    this.eventsDate = props.match.params.date;

    console.log('EventTablePage: ', this.eventsDate);
    this.state = {
      query: {
        FromIndex: 0,
        QueriedQuantity: 10,
  ...
  private async getEvents () {
    try {
      const events:TEventItems = await EventReader.getDateEvents(this.eventsDate);
  ...
  private isNextPossible (direction: IQueryDirection): boolean {
    return !((direction === IQueryDirection.Next) && (this.state.respond.ItemsAfter === 0));
  }
  ...
  private tougleDateSortDirection (direction: ISortDirection): ISortDirection {
    return (direction === ISortDirection.Up) ? ISortDirection.Down : ISortDirection.Up;
  }
  ...
  private tougleEventsSortMode (mode: IEventSortMode): IEventSortMode {
    if (mode === IEventSortMode.All) {
      return IEventSortMode.Alarm
    };
    if (mode === IEventSortMode.Alarm) {
      return IEventSortMode.Warning
    };
    if (mode === IEventSortMode.Warning) {
      return IEventSortMode.Info
    };
    if (mode === IEventSortMode.Info) {
  ...
  private handlerFilterFormClose (range: ISearchRangeQuery | undefined) {
    (range) ? this.enableFilter(range) : this.disableFilter();
  }

  private getAvalibleTimeRangeInHHMMSS ():Array<string> {
    return this.eventsModel?.AvaliableTimeRange || [];
  }
