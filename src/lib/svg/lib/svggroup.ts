...
class TTree {
    ...
    getRootNode (element: any): any {
        ...
        while (i !== 0) {
            i--;
            const c = this.node.children[i].data; // вытащу для отладки
            if (c === element) {
    ...
    getNode = function (node: TNode, element: any): any {
        ...
        while (i !== 0) {
            i--;
            const c = node.children[i].data; // вытащу для отладки
            if (c === element) {
    ...
    addNode (e: any) {
        ...
        let i = a.length; // кол-во найденных родителей
...
function addArrToTree (a: Array<any>, treeNode: TNode) {
    let i: number = 0;
    let searchNode: TNode = treeNode; // с чего начинать поиск
    while (i < a.length) {
        const p = a[i]; // получил старшего родителя из списка
        ...
        (function (p, node) {
            let j = node.children.length; // кол-во детей у ноды
            let n; // найденная нода (или undefine если не нашёл)
            while (j !== 0) {
                j--;
...
function getSvgTransfList (attr: Array<string>): Array<TSvgTransformList> {
    ...
    let i = attr.length;
    const arr: Array<TSvgTransformList> = [];
    while (i !== 0) {
        i--;
        ...
        const s = attr[i].split(/[, ()]/);
        const t: TSvgTransformList = new TSvgTransformList(s[0]);
...
function getSvgTransfStrArr (s: string): Array<string> {
    ...
    let j = aTrs.length; // я учитываю Три команды трансформации
    while (j !== 0) { // найду вхождения каждой команды так как их может быть несколько
        j--;
        const target = aTrs[j]; // цель поиска
        let pos = -1;
...
function getTransformation (
    ...
    const result: TSvgTransformation = { dX, dY, angle, sX, sY };
    if (s !== null) {
        const aTrStrList: Array<string> = getSvgTransfStrArr(s); // получу массив строк трансформаций
        const a = getSvgTransfList(aTrStrList);
        let i = a.length;
        while (i !== 0) {
            i--;
            const o = a[i];
...
function getTransform (treeNode: TNode) {
    (function recurse (currentNode: any) {
        ...
        t = getTransformation(
            s,
            p.dX,
            p.dY, // чтобы не изменять parentTransform
            p.angle,
            p.sX,
            p.sY
        );
    ...
    if (scr !== null) {
        const ctx = scr.getContext('2d');
            (function recurse (currentNode) {
                ...
                let i: number = currentNode.children.length;
                while (i !== 0) {
                    i--;
                    recurse(currentNode.children[i]);
                }
...
// П отом выявить его родителя - это будет группа в которой находится элемент.
...
export class TSVGGroups {
    ...
    private getGroupsArr (id: string) {
        ...
        let i = 0; ; // кол-во найденных тегов
        if (gs.length !== 0) {
            i = gs.length;
            while (i !== 0) {
                i--;
    ...
    public inBound (x: number, y: number, tolerance: number) {
        ...
        let treeNode: TNode = this.tree._root;
