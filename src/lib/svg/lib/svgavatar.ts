...
export class ASvgTransf {
	...
	getSvgTransfStrArr (s: string): Array<string> {
		...
		const res: Array<string> = []; // тут будут строки трансформаций (потом их разделю на тип и параметры (которые в скобках))
		let j: number = aTrs.length; // я учитываю Три команды трансформации
		while (j !== 0) { // найду вхождения каждой команды так как их может быть несколько
			j--;
			const target: string = aTrs[j]; // цель поиска
			let pos: number = -1;
			while ((pos = s.indexOf(target, pos + 1)) !== -1) {
				const ts = s.slice(pos, s.indexOf(')', pos + target.length) + 1); // команды трансформации закрываются скобкой ")"
				res.push(ts);
			}
		}
		return res;
	...
	getSvgTransfList (sattr: Array<string>) {
		...
		while (i !== 0) {
			...
			const s = sattr[i].split(/[, ()]/);
			// console.warn(s);
			const t = new TSvgTransformList(s[0]);
			for (let j = 1; j < s.length; j++) { // покажу что получил console.warn(s[j])
				if (s[j] !== "") {
					t.param.push(s[j]);
				}
			}
...
function getSvgParentTransformation (parent: any): TTransformation {
	if (parent !== undefined) {
		const p: any = parent.getAttribute('transform'); // поиск аттрибута transform, в s будет строка аттрибута
		if (p != null) { // у объекта есть родитель (элемент более высокго уровня)
			const aSvgTransf = new ASvgTransf(p);
	...
	return defaultTransformation; // значение по умолчанию
}
...
export class TFigurePath extends TFigure {
	...
	constructor (owner: any, t: TTransformation) {
		...
		this.t = t;
		this.parseDAttr();
		this.boundRect = path2BoundRect(this.points); // определяю Границу фигуры
	}
	...
	private parseDAttr () {
		...
		const s: Array<string> = this.owner.getAttribute('d').split(/ /);
		// console.warn(s);

		let i = 0;
		// параметр в котором первый символ может быть: "М" или "L" или "A"
		// М-это первая точка Мx y
		// L - линия Lx y
		// А - дуга A rx ry x-axis-rotation large-arc-flag sweep-flag x y
		while (i !== s.length) {
			const m = s[i++];
			const mode = m.charAt(0);
			let	x = 0;
			let y = 0;
			let p: TPoint = { x: 0, y: 0 };
			// отработка М, L, A
				switch (mode) {
					case 'M':
					case 'L':
						...
						const res: TModePoint = { mode, x, y };
						this.points.push(res);
						break;
					case 'A':
						// d=".... A15.7162 13.0481 -55.83   0   0   20.04 114.34"
						// Arx  ry  x-axis-rotation large-arc-flag sweep-flag x  y
						const rx = Number(m.slice(1)); // радиус по Х
						const ry = Number(s[i++]); // радиус по Y
						const xAxisRotation = Number(s[i++]);
						const largeArcFlag = Number(s[i++]);
						const sweepFlag = Number(s[i++]);
...
export class TFigureRect extends TFigure {
	...
	constructor (owner: any, t: TTransformation) {
		...
		this.points.push({
			mode: 'M', // установить нач.точку
			x: this.Left,
			y: this.Top
		});
		this.points.push({
			mode: 'L', // установить следующую точку
		...
		this.points.push({
			mode: 'L', // возврат к начальной точке
			x: this.Left,
			y: this.Top
		});
		// теперь вращаю и смещаю координаты всех точек
		this.points.forEach(function (item) {
			const p = rotateXY(item.x, item.y, t.angle);
			item.x = p.x + t.dX;
			item.y = p.y + t.dY;
			}, this);
...
function searchSvgViewObjects (svgID: string) {
	const s: any = document.getElementById(svgID); // получаю доступ к DOM SVG
	const gElements: any = s.contentDocument.children[0].getElementsByTagName('*'); // найду все элементы
	// теперь просканирую список в поисках отображаемых граф. элементов
	let i = gElements.length; // кол-во найденных элементов
	let svgElement: any = {}; // объект элемента
	const res: Array<any> = []; // массив найденных элементов
	while (i !== 0) {
		i--;
		svgElement = gElements[i];
...
function createSvgAvatarObjects (aVO: Array<any>): Array<any> { // aVO - массив отображаемых элементов
	const res: Array<any> = []; // массив объектов-Аватаров
	let o: any = {}; // объект для SVG элемента
	let i: number = aVO.length;
	while (i !== 0) {
		i--;
		o = aVO[i];
		// получить трансформации SVG для объекта
		const t: TTransformation = getSvgParentTransformation(o.parentElement); // получить трансформации родителя
		switch (o.tagName) {
